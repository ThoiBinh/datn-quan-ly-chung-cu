...
@section('title', 'Loại yêu cầu')
@section('page-title', 'Loại yêu cầu')
...
<div class="space-y-5">
    ...
    <div class="grid grid-cols-3 gap-4">
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-4 flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-xs text-gray-500">Tổng loại yêu cầu</p>
            </div>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-4 flex items-center gap-4">
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-4 flex items-center gap-4">
            ...
            <div>
                <p class="text-2xl font-bold text-red-500">{{ $tongDaXoa }}</p>
    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm">
        <div class="p-4 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <form method="GET" action="{{ route('admin.loai-yeu-cau.index') }}" class="flex flex-wrap items-center gap-2">
                <div class="relative">
                ...
                <a href="{{ route('admin.loai-yeu-cau.index') }}" class="px-3 py-2 border border-gray-200 text-gray-500 text-sm rounded-lg hover:bg-gray-50">Xóa lọc</a>
                @endif
            </form>

            <a href="{{ route('admin.loai-yeu-cau.create') }}"
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="bg-gray-50 border-b border-gray-200">
                        <th class="text-left px-4 py-3 font-semibold text-gray-600 text-xs uppercase tracking-wider">Tên loại yêu cầu</th>
                        <th class="text-center px-4 py-3 font-semibold text-gray-600 text-xs uppercase tracking-wider">Số yêu cầu</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors {{ $item->deletedAt ? 'opacity-60' : '' }}">
                        <td class="px-4 py-3">
                            <div class="flex items-center gap-2">
                                <div class="w-7 h-7 rounded-lg {{ $item->deletedAt ? 'bg-gray-100' : 'bg-blue-100' }} flex items-center justify-center flex-shrink-0">
                                    <svg class="w-4 h-4 {{ $item->deletedAt ? 'text-gray-400' : 'text-blue-600' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/>
                                    </svg>
                                </div>
                        ...
                        <td class="px-4 py-3 text-center">
                            <span class="inline-flex items-center justify-center w-8 h-8 rounded-full text-xs font-semibold
                                {{ $item->yeu_cau_count > 0 ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-500' }}">
                                {{ $item->yeu_cau_count }}
                            </span>
                        </td>
                        <td class="px-4 py-3 text-gray-600 text-xs">
                            {{ $item->nguoiCapNhat?->ho_ten ?? '—' }}
                        </td>
                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center justify-center gap-1">
                                @if($item->deletedAt)
                                    <form action="{{ route('admin.loai-yeu-cau.restore', $item->id) }}" method="POST" class="inline">
                                        @csrf @method('PATCH')
                                ...
                                @else
                                    <a href="{{ route('admin.loai-yeu-cau.show', $item) }}" class="p-1.5 text-blue-600 hover:bg-blue-50 rounded-lg" title="Xem">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                                        </svg>
                                    </a>
                                    <a href="{{ route('admin.loai-yeu-cau.edit', $item) }}" class="p-1.5 text-amber-600 hover:bg-amber-50 rounded-lg" title="Sửa">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                        </svg>
                                    </a>
                                    <button type="button"
                                            @click="$dispatch('open-delete', { url: '{{ route('admin.loai-yeu-cau.destroy', $item) }}', name: '{{ addslashes($item->name) }}' })"
                    ...
                    <tr>
                        <td colspan="6" class="px-4 py-12 text-center text-gray-400 text-sm">
                            Không tìm thấy loại yêu cầu nào
                        </td>
                    </tr>
                    @endforelse
                </tbody>
    ...
    <div x-data="{ open: false, url: '', name: '' }"
        ...
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm mx-auto p-6" @click.stop>
            <div class="text-center mb-5">
                ...
                <p class="font-semibold text-gray-800">Xóa loại yêu cầu?</p>
                <p class="text-sm text-gray-500 mt-1">Loại <span class="font-medium text-gray-700" x-text="`«${name}»`"></span> sẽ bị ẩn (có thể khôi phục).</p>
            </div>
