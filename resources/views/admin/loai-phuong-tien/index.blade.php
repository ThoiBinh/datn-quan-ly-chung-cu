...
@section('title', 'Loại phương tiện')
@section('page-title', 'Loại phương tiện')
...
<div class="space-y-5">
    ...
    <div class="grid grid-cols-2 gap-4">
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-4 flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-xs text-gray-500">Tổng loại phương tiện</p>
            </div>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-4 flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-xs text-gray-500">Loại đang có phương tiện</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm">
        <div class="p-4 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <form method="GET" action="{{ route('admin.loai-phuong-tien.index') }}" class="flex flex-wrap items-center gap-2">
                <div class="relative">
                ...
                <select name="su_dung" class="px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                    ...
                    <option value="co" {{ request('su_dung') === 'co' ? 'selected' : '' }}>Có phương tiện</option>
                    <option value="khong" {{ request('su_dung') === 'khong' ? 'selected' : '' }}>Chưa có phương tiện</option>
                </select>
                ...
                <select name="sort" class="px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                    ...
                    <option value="ten_loai_phuong_tien" {{ request('sort') === 'ten_loai_phuong_tien' ? 'selected' : '' }}>Tên A-Z</option>
                </select>
                ...
                <a href="{{ route('admin.loai-phuong-tien.index') }}" class="px-3 py-2 border border-gray-200 text-gray-500 text-sm rounded-lg hover:bg-gray-50">Xóa lọc</a>
                @endif
            </form>

            <a href="{{ route('admin.loai-phuong-tien.create') }}"
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="bg-gray-50 border-b border-gray-200">
                        <th class="text-left px-4 py-3 font-semibold text-gray-600 text-xs uppercase tracking-wider">Tên loại phương tiện</th>
                        <th class="text-center px-4 py-3 font-semibold text-gray-600 text-xs uppercase tracking-wider">Số phương tiện</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        <td class="px-4 py-3">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="font-medium text-gray-800">{{ $item->ten_loai_phuong_tien }}</span>
                            </div>
                        </td>
                        <td class="px-4 py-3 text-center">
                            <span class="inline-flex items-center justify-center w-8 h-8 rounded-full text-xs font-semibold
                                {{ $item->phuong_tien_count > 0 ? 'bg-sky-100 text-sky-700' : 'bg-gray-100 text-gray-500' }}">
                                {{ $item->phuong_tien_count }}
                            </span>
                        </td>
                        <td class="px-4 py-3 text-center">
                            @if($item->phuong_tien_count > 0)
                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center justify-center gap-1">
                                <a href="{{ route('admin.loai-phuong-tien.show', $item) }}" class="p-1.5 text-blue-600 hover:bg-blue-50 rounded-lg" title="Xem">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                                    </svg>
                                </a>
                                <a href="{{ route('admin.loai-phuong-tien.edit', $item) }}" class="p-1.5 text-amber-600 hover:bg-amber-50 rounded-lg" title="Sửa">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                @if($item->phuong_tien_count === 0)
                                <button type="button"
                                        @click="$dispatch('open-delete', { url: '{{ route('admin.loai-phuong-tien.destroy', $item) }}', name: '{{ addslashes($item->ten_loai_phuong_tien) }}' })"
                                ...
                                @else
                                <span class="p-1.5 text-gray-300 cursor-not-allowed" title="Không thể xóa — đang có phương tiện">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                                    </svg>
                                </span>
                    ...
                    <tr>
                        <td colspan="4" class="px-4 py-12 text-center text-gray-400 text-sm">
                            Không tìm thấy loại phương tiện nào
                        </td>
                    </tr>
                    @endforelse
                </tbody>
    ...
    <div x-data="{ open: false, url: '', name: '' }"
        ...
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm mx-auto p-6" @click.stop>
            <div class="text-center mb-5">
                ...
                <p class="font-semibold text-gray-800">Xóa loại phương tiện?</p>
