...
<div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
    <div class="bg-amber-50 border border-amber-200 rounded-xl p-4">
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-xl font-bold text-amber-700">{{ $stats['hoa_don_chua_tt'] }}</p>
            </div>
        </div>
    </div>
    <div class="bg-red-50 border border-red-200 rounded-xl p-4">
        <div class="flex items-center gap-3">
            ...
            <div>
                <p class="text-xs text-red-600 font-medium">Hóa đơn quá hạn</p>
                <p class="text-xl font-bold text-red-700">{{ $stats['hoa_don_qua_han'] }}</p>
            </div>
        </div>
    </div>
    <div class="bg-blue-50 border border-blue-200 rounded-xl p-4">
        <div class="flex items-center gap-3">
            ...
            <div>
                <p class="text-xs text-blue-600 font-medium">Yêu cầu mới</p>
                <p class="text-xl font-bold text-blue-700">{{ $stats['yeu_cau_moi'] }}</p>
            </div>
        </div>
    </div>
</div>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
        <h3 class="font-semibold text-gray-800 mb-4">Yêu cầu mới nhất</h3>
        <div class="space-y-3">
            @forelse($yeuCauMoi as $yc)
            <div class="flex items-start gap-3 p-3 rounded-lg bg-gray-50 hover:bg-gray-100 transition-colors">
                <div class="w-8 h-8 rounded-full bg-blue-100 flex items-center justify-center text-blue-700 text-sm font-medium flex-shrink-0">
            ...
            @empty
            <p class="text-center text-gray-400 text-sm py-6">Không có yêu cầu mới</p>
            @endforelse
        </div>
        @if(count($yeuCauMoi))
