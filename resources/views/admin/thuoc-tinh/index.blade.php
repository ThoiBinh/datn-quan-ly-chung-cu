...
<div class="space-y-5">
    ...
    <div class="grid grid-cols-2 gap-4">
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-4 flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-xs text-gray-500">Đã xóa (có thể khôi phục)</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm">
        <div class="p-4 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <form method="GET" action="{{ route('admin.thuoc-tinh.index') }}" class="flex flex-wrap items-center gap-2">
                {{-- Search --}}
                <div class="relative">
                    <svg class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0"/>
                    </svg>
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Tìm tên thuộc tính..."
                ...
                <select name="sort" class="px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                    <option value="createdAt" {{ request('sort', 'createdAt') === 'createdAt' ? 'selected' : '' }}>Ngày tạo</option>
                    <option value="ten_thuoc_tinh" {{ request('sort') === 'ten_thuoc_tinh' ? 'selected' : '' }}>Tên A-Z</option>
                    <option value="updatedAt" {{ request('sort') === 'updatedAt' ? 'selected' : '' }}>Cập nhật</option>
                </select>
                <select name="direction" class="px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                    <option value="desc" {{ request('direction', 'desc') === 'desc' ? 'selected' : '' }}>Mới nhất</option>
                    <option value="asc" {{ request('direction') === 'asc' ? 'selected' : '' }}>Cũ nhất</option>
                </select>

                {{-- Hiển thị đã xóa --}}
                <label class="flex items-center gap-1.5 text-sm text-gray-600 cursor-pointer select-none">
                    <input type="checkbox" name="show_trashed" value="1" {{ request()->boolean('show_trashed') ? 'checked' : '' }}
                ...
                @if(request()->hasAny(['search', 'show_trashed', 'sort', 'direction']))
                <a href="{{ route('admin.thuoc-tinh.index') }}" class="px-3 py-2 border border-gray-200 text-gray-500 text-sm rounded-lg hover:bg-gray-50">Xóa lọc</a>
                @endif
            </form>

            <a href="{{ route('admin.thuoc-tinh.create') }}"
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="bg-gray-50 border-b border-gray-200">
                        ...
                        <th class="text-center px-4 py-3 font-semibold text-gray-600 text-xs uppercase tracking-wider">Số căn hộ dùng</th>
                    ...
                    <tr class="hover:bg-gray-50 transition-colors {{ $item->deletedAt ? 'opacity-60' : '' }}">
                        <td class="px-4 py-3">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="font-medium text-gray-800">{{ $item->ten_thuoc_tinh }}</span>
                            </div>
                        </td>
                        <td class="px-4 py-3 text-center">
                            <span class="inline-flex items-center justify-center w-7 h-7 rounded-full text-xs font-semibold
                                {{ $item->can_ho_count > 0 ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-500' }}">
                                {{ $item->can_ho_count }}
                            </span>
                        </td>
                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center justify-center gap-1">
                                @if($item->deletedAt)
                                <form action="{{ route('admin.thuoc-tinh.restore', $item->id) }}" method="POST">
                                    @csrf @method('PATCH')
                                ...
                                @else
                                <a href="{{ route('admin.thuoc-tinh.show', $item) }}" class="p-1.5 text-blue-600 hover:bg-blue-50 rounded-lg" title="Xem">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                                    </svg>
                                </a>
                                <a href="{{ route('admin.thuoc-tinh.edit', $item) }}" class="p-1.5 text-amber-600 hover:bg-amber-50 rounded-lg" title="Sửa">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                    </svg>
                                </a>
                                <button type="button"
                                        @click="$dispatch('open-delete', { url: '{{ route('admin.thuoc-tinh.destroy', $item) }}', name: '{{ addslashes($item->ten_thuoc_tinh) }}' })"
        ...
        @if($dsThuocTinh->hasPages())
        <div class="px-4 py-3 border-t border-gray-100">{{ $dsThuocTinh->links() }}</div>
        @endif
    </div>
    ...
    <div x-data="{ open: false, url: '', name: '' }"
        ...
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm mx-auto p-6" @click.stop>
            <div class="text-center mb-5">
                ...
                <p class="font-semibold text-gray-800">Xóa thuộc tính?</p>
                <p class="text-sm text-gray-500 mt-1">Thuộc tính <span class="font-medium text-gray-700" x-text="`«${name}»`"></span> sẽ bị xóa tạm thời và có thể khôi phục.</p>
            </div>
