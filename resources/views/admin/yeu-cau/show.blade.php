...
<div class="max-w-4xl space-y-5" x-data="{ showReject: {{ $errors->has('ly_do_tu_choi') ? 'true' : 'false' }} }">
    ...
    <nav class="flex items-center gap-2 text-sm text-gray-500">
        <a href="{{ route('admin.yeu-cau.index') }}" class="hover:text-blue-600 transition-colors">Yêu cầu cư dân</a>
        ...
        <div class="lg:col-span-2 space-y-5">
            ...
            <div class="bg-white rounded-2xl border border-gray-200 shadow-sm">
                ...
                <div class="px-6 py-5">
                    ...
                    {{-- Hiển thị dữ liệu phương tiện dạng grid thay vì JSON thô --}}
            ...
            <div class="bg-white rounded-2xl border border-gray-200 shadow-sm">
                ...
                <div class="px-6 py-5">
                    ...
                    <div class="flex items-start gap-4">
                        <div class="w-11 h-11 rounded-full bg-blue-100 flex items-center justify-center text-blue-700 font-bold flex-shrink-0">
        ...
        <div class="space-y-5">
            ...
            <div class="bg-white rounded-2xl border border-blue-200 shadow-sm">
                <div class="px-5 py-4 border-b border-blue-100">
                    <h2 class="text-sm font-semibold text-blue-700 uppercase tracking-wide">Xét duyệt phương tiện</h2>
                </div>
                <div class="px-5 py-4 space-y-3">
                    {{-- Nút Duyệt --}}
                    <form method="POST" action="{{ route('admin.yeu-cau.approve', $yeuCau) }}">
                        @csrf @method('PATCH')
                        <button type="submit"
                    ...
                    <div x-show="showReject" x-cloak>
                        <form method="POST" action="{{ route('admin.yeu-cau.reject', $yeuCau) }}" class="space-y-3">
                            @csrf @method('PATCH')
            ...
            {{-- Update form thông thường (không hiện khi đang xử lý duyệt/từ chối PT) --}}
            @if(!($isDangKyPT && $yeuCau->trang_thai == \App\Models\YeuCauCuDan::TRANG_THAI_MOI))
            <div class="bg-white rounded-2xl border border-gray-200 shadow-sm">
                <div class="px-5 py-4 border-b border-gray-100">
                    <h2 class="text-sm font-semibold text-gray-600 uppercase tracking-wide">Cập nhật</h2>
                </div>
                <form method="POST" action="{{ route('admin.yeu-cau.update', $yeuCau) }}" class="px-5 py-4 space-y-4">
                    @csrf @method('PUT')
                    ...
                    <button type="submit"
                            class="w-full px-4 py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold rounded-xl transition-colors">
                        Lưu cập nhật
                    </button>
                </form>
            </div>
            ...
            <a href="{{ route('admin.yeu-cau.index') }}"
