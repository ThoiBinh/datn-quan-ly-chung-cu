...
<div class="space-y-6">
    ...
    <div class="flex flex-wrap items-center gap-3">
        <a href="{{ route('admin.thong-bao.edit', $thongBao) }}"
           class="inline-flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-lg transition-colors">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
            Chỉnh sửa
        </a>
        @unless($thongBao->trashed())
        <form action="{{ route('admin.thong-bao.toggle-hide', $thongBao) }}" method="POST" onsubmit="return confirm('Ẩn thông báo này?')">
            @csrf @method('PATCH')
        ...
        @else
        <form action="{{ route('admin.thong-bao.restore', $thongBao->id) }}" method="POST" onsubmit="return confirm('Khôi phục?')">
            @csrf @method('PATCH')
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-4 py-3">
                            @if($vaiTro)
                            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-indigo-100 text-indigo-700">{{ $vaiTro->vai_tro }}</span>
