...
@section('title', 'Trạng thái căn hộ')
@section('page-title', 'Trạng thái căn hộ')
...
<div class="space-y-5">
    ...
    <div class="grid grid-cols-2 gap-4">
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-4 flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-xs text-gray-500">Tổng trạng thái căn hộ</p>
            </div>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-4 flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-xs text-gray-500">Trạng thái đang có căn hộ</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm">
        <div class="p-4 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <form method="GET" action="{{ route('admin.trang-thai-can-ho.index') }}" class="flex flex-wrap items-center gap-2">
                <div class="relative">
                    <svg class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0"/>
                    </svg>
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Tìm tên trạng thái..."
                ...
                <select name="sort" class="px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                    ...
                    <option value="ten_trang_thai" {{ request('sort') === 'ten_trang_thai' ? 'selected' : '' }}>Tên A-Z</option>
                </select>
                ...
                <a href="{{ route('admin.trang-thai-can-ho.index') }}" class="px-3 py-2 border border-gray-200 text-gray-500 text-sm rounded-lg hover:bg-gray-50">Xóa lọc</a>
                @endif
            </form>

            <a href="{{ route('admin.trang-thai-can-ho.create') }}"
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="bg-gray-50 border-b border-gray-200">
                        <th class="text-left px-4 py-3 font-semibold text-gray-600 text-xs uppercase tracking-wider">Tên trạng thái</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        <td class="px-4 py-3">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="font-medium text-gray-800">{{ $item->ten_trang_thai }}</span>
                            </div>
                        </td>
                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center justify-center gap-1">
                                <a href="{{ route('admin.trang-thai-can-ho.show', $item) }}" class="p-1.5 text-blue-600 hover:bg-blue-50 rounded-lg" title="Xem">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                                    </svg>
                                </a>
                                <a href="{{ route('admin.trang-thai-can-ho.edit', $item) }}" class="p-1.5 text-amber-600 hover:bg-amber-50 rounded-lg" title="Sửa">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                @if($item->can_ho_count === 0)
                                <button type="button"
                                        @click="$dispatch('open-delete', { url: '{{ route('admin.trang-thai-can-ho.destroy', $item) }}', name: '{{ addslashes($item->ten_trang_thai) }}' })"
                    ...
                    <tr>
                        <td colspan="4" class="px-4 py-12 text-center text-gray-400 text-sm">
                            Không tìm thấy trạng thái căn hộ nào
                        </td>
                    </tr>
                    @endforelse
                </tbody>
        ...
        @if($dsTrangThai->hasPages())
        <div class="px-4 py-3 border-t border-gray-100">{{ $dsTrangThai->links() }}</div>
        @endif
    </div>
    ...
    <div x-data="{ open: false, url: '', name: '' }"
        ...
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm mx-auto p-6" @click.stop>
            <div class="text-center mb-5">
                ...
                <p class="font-semibold text-gray-800">Xóa trạng thái căn hộ?</p>
                <p class="text-sm text-gray-500 mt-1">Trạng thái <span class="font-medium text-gray-700" x-text="`«${name}»`"></span> sẽ bị xóa vĩnh viễn.</p>
            </div>
