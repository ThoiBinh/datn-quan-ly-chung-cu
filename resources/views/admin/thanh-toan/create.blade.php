...
<div class="max-w-2xl mx-auto space-y-5">
    ...
    <nav class="flex items-center gap-1.5 text-sm text-gray-500 dark:text-slate-400">
        ...
        <a href="{{ route('admin.hoa-don.show', $hoaDon) }}" class="font-mono hover:text-indigo-600 transition-colors">{{ $hoaDon->ma_thanh_toan }}</a>
        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
        <span class="text-gray-700 dark:text-slate-200">Thanh toán</span>
    </nav>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm p-5">
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm mb-4">
            ...
            <div>
                ...
                <p class="font-semibold text-gray-800 dark:text-slate-200">Tháng {{ $hoaDon->thang }}/{{ $hoaDon->nam }}</p>
            </div>
            <div>
        ...
        <div class="grid grid-cols-3 gap-3 pt-4 border-t border-gray-100 dark:border-slate-700">
            <div class="rounded-lg bg-gray-50 dark:bg-slate-700/50 p-3 text-center">
                <p class="text-xs text-gray-400 dark:text-slate-500 mb-0.5">Tổng tiền HĐ</p>
                <p class="font-bold text-gray-900 dark:text-white tabular-nums text-sm">{{ number_format($hoaDon->tong_tien ?? 0, 0, ',', '.') }}đ</p>
            </div>
            <div class="rounded-lg bg-emerald-50 dark:bg-emerald-900/20 p-3 text-center">
                ...
                <p class="font-bold text-emerald-700 dark:text-emerald-300 tabular-nums text-sm">{{ number_format($hoaDon->so_tien_da_thanh_toan ?? 0, 0, ',', '.') }}đ</p>
            </div>
            <div class="rounded-lg bg-red-50 dark:bg-red-900/20 p-3 text-center">
                ...
                <p class="font-bold text-red-700 dark:text-red-300 tabular-nums text-sm">{{ number_format($conNo, 0, ',', '.') }}đ</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm overflow-hidden">
        <div class="flex items-center gap-2.5 px-5 py-4 border-b border-gray-100 dark:border-slate-700">
            ...
            <h2 class="text-sm font-semibold text-gray-800 dark:text-slate-200">Thông tin thanh toán</h2>
        </div>
        <form method="POST" action="{{ route('admin.thanh-toan.store', $hoaDon) }}" class="px-5 py-5 space-y-4"
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <p class="text-xs text-gray-400 dark:text-slate-500 mt-1">Tối đa: <span class="font-medium text-red-600 dark:text-red-400">{{ number_format($conNo, 0, ',', '.') }}đ</span></p>
                </div>
                <div>
            ...
            <div>
                ...
                <textarea name="ghi_chu" rows="3" maxlength="500"
                          ...
                          class="w-full px-3 py-2.5 border border-gray-300 dark:border-slate-600 rounded-lg text-sm bg-white dark:bg-slate-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-400 resize-none">{{ old('ghi_chu') }}</textarea>
            </div>
            <div class="flex items-center gap-3 pt-2">
                <a href="{{ route('admin.hoa-don.show', $hoaDon) }}"
                   class="flex-1 text-center py-2.5 border border-gray-300 dark:border-slate-600 text-gray-700 dark:text-slate-300 text-sm font-medium rounded-xl hover:bg-gray-50 dark:hover:bg-slate-700 transition-colors">
                    Hủy bỏ
                </a>
                <button type="submit"
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm overflow-hidden">
        <div class="px-5 py-4 border-b border-gray-100 dark:border-slate-700">
            <h3 class="text-sm font-semibold text-gray-800 dark:text-slate-200">Giao dịch trước đó</h3>
        </div>
        <div class="divide-y divide-gray-50 dark:divide-slate-700/50">
            @foreach($hoaDon->lichSuThanhToan as $ls)
            <div class="px-5 py-3 flex items-center justify-between gap-3">
                <div>
                    <p class="text-sm font-semibold text-emerald-600 dark:text-emerald-400 tabular-nums">{{ number_format($ls->so_tien ?? 0, 0, ',', '.') }}đ</p>
                    <p class="text-xs text-gray-400 dark:text-slate-500">{{ $ls->ngay_thanh_toan?->format('d/m/Y') ?? '—' }}
