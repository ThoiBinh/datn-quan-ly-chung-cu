...
$statusConfig = match($hoaDon->trang_thai) {
    ...
    3 => ['label' => 'Trễ hạn',         'dot' => 'bg-red-500',     'text' => 'text-red-600',     'bg' => 'bg-red-50'],
        ...
        <div class="flex items-center gap-2 flex-wrap">
            ...
            @if(in_array($hoaDon->trang_thai, [1, 2, 3]))
                ...
                <span title="Hóa đơn đã thanh toán đầy đủ.">
                    <button disabled class="inline-flex items-center gap-2 px-3.5 py-2 rounded-lg text-sm font-medium bg-indigo-50 text-indigo-400 border border-indigo-200 opacity-60 cursor-not-allowed">
    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
            ...
            <div>
                ...
                <p class="text-sm font-bold text-gray-900 tabular-nums" x-text="fmtMoney(tongTienHoaDon)">{{ number_format($hoaDon->tong_tien ?? 0, 0, ',', '.') }}đ</p>
            </div>
            <div>
        ...
        <div class="lg:col-span-2 space-y-5">
                ...
                <table class="w-full text-sm">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-4 py-2.5 text-right text-xs font-medium text-gray-500">Số lượng</th>
                    ...
                    <tfoot class="border-t border-gray-200 bg-gray-50">
                        <tr>
                            <td colspan="{{ !$isDaTT ? 3 : 2 }}" class="px-5 py-3 text-sm font-semibold text-gray-700 text-right">Tổng cộng</td>
            ...
            <div class="grid grid-cols-3 gap-4">
                <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-4 text-center">
                    <p class="text-xs text-gray-400 mb-1">Tổng tiền HĐ</p>
                    <p class="text-lg font-bold text-gray-900 tabular-nums" x-text="fmtMoney(tongTienHoaDon)">{{ number_format($hoaDon->tong_tien ?? 0, 0, ',', '.') }}đ</p>
                </div>
                <div class="bg-white rounded-xl border border-emerald-200 shadow-sm p-4 text-center">
                    <p class="text-xs text-emerald-600 mb-1">Đã thanh toán</p>
                    <p class="text-lg font-bold text-emerald-600 tabular-nums" x-text="fmtMoney(daTTHoaDon)">{{ number_format($hoaDon->so_tien_da_thanh_toan ?? 0, 0, ',', '.') }}đ</p>
                </div>
                <div class="bg-white rounded-xl shadow-sm p-4 text-center border"
                     :class="getConNo() > 0 ? 'border-red-200' : 'border-emerald-200'">
                    <p class="text-xs mb-1" :class="getConNo() > 0 ? 'text-red-600' : 'text-emerald-600'">Còn nợ</p>
            ...
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                <div class="flex items-center gap-2.5 px-5 py-4 border-b border-gray-100">
                    ...
                    <span class="ml-auto text-xs text-gray-400">{{ $hoaDon->lichSuThanhToan->count() }} giao dịch</span>
                    <a href="{{ route('manager.thanh-toan.index', ['hoa_don_id' => $hoaDon->id]) }}"
                       class="text-xs text-indigo-600 hover:underline">Xem tất cả</a>
                </div>
                ...
                <table class="w-full text-sm">
                    <thead class="bg-gray-50">
                        <tr>
                            <th class="px-4 py-2.5 text-left text-xs font-medium text-gray-500 w-10">STT</th>
                            <th class="px-4 py-2.5 text-left text-xs font-medium text-gray-500">Ngày TT</th>
                            <th class="px-4 py-2.5 text-right text-xs font-medium text-gray-500">Số tiền</th>
                            <th class="px-4 py-2.5 text-left text-xs font-medium text-gray-500">Phương thức</th>
                            <th class="px-4 py-2.5 text-left text-xs font-medium text-gray-500">Nguồn</th>
                            <th class="px-4 py-2.5 text-left text-xs font-medium text-gray-500">Ghi chú</th>
                    ...
                    <tbody class="divide-y divide-gray-50">
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3 text-gray-700 whitespace-nowrap">{{ $ls->ngay_thanh_toan?->format('d/m/Y') ?? '—' }}</td>
                            ...
                            <td class="px-4 py-3 text-xs text-gray-500 max-w-[120px] truncate" title="{{ $ls->ghi_chu }}">{{ $ls->ghi_chu ?? '—' }}</td>
                    ...
                    <tfoot class="border-t-2 border-gray-200 bg-gray-50">
                        <tr>
                            <td colspan="6" class="px-4 py-3 text-xs font-semibold text-gray-700 text-right">Tổng đã thanh toán</td>
                            <td class="px-4 py-3 text-right font-bold text-emerald-600 tabular-nums" x-text="fmtMoney(daTTHoaDon)">{{ number_format($hoaDon->so_tien_da_thanh_toan ?? 0, 0, ',', '.') }}đ</td>
                        </tr>
                    </tfoot>
                </table>
                </div>
                ...
                <div class="px-5 py-8 text-center">
                    ...
                    <p class="text-sm text-gray-400">Chưa có giao dịch thanh toán nào</p>
                    @if($isCanTT)
                    <button @click="showThanhToan = true" type="button"
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden"
            ...
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                <div class="flex items-center gap-3">
                    ...
                    <h3 class="text-base font-bold text-gray-900">Ghi nhận thanh toán</h3>
                </div>
                <button @click="showThanhToan = false" class="w-8 h-8 rounded-full flex items-center justify-center text-gray-400 hover:text-gray-600 hover:bg-gray-100 transition-colors">
            ...
            <div class="px-6 py-4 bg-gray-50 border-b border-gray-100">
                <div class="grid grid-cols-2 gap-3 text-sm">
                    <div>
                        ...
                        <p class="font-mono font-semibold text-indigo-600">{{ $hoaDon->ma_thanh_toan }}</p>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <p class="font-semibold text-gray-900 tabular-nums" x-text="fmtMoney(tongTienHoaDon)">{{ number_format($hoaDon->tong_tien ?? 0, 0, ',', '.') }}đ</p>
                    </div>
                </div>
                <div class="mt-3 pt-3 border-t border-gray-200 grid grid-cols-2 gap-3">
                    <div class="bg-emerald-50 rounded-lg px-3 py-2">
                        ...
                        <p class="font-bold text-emerald-700 tabular-nums" x-text="fmtMoney(daTTHoaDon)">{{ number_format($hoaDon->so_tien_da_thanh_toan ?? 0, 0, ',', '.') }}đ</p>
                    </div>
                    <div class="bg-red-50 rounded-lg px-3 py-2">
                        <p class="text-xs text-red-600 mb-0.5">Còn nợ</p>
                        <p class="font-bold text-red-700 tabular-nums" x-text="fmtMoney(getConNo())">{{ number_format($conNo, 0, ',', '.') }}đ</p>
                    </div>
                </div>
            </div>

            {{-- Payment form --}}
            <form method="POST" action="{{ route('manager.thanh-toan.store', $hoaDon) }}" class="px-6 py-5 space-y-4">
                @csrf
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <p class="text-xs text-gray-400 mt-1">Tối đa: <span class="font-medium text-red-600" x-text="fmtMoney(getConNo())">{{ number_format($conNo, 0, ',', '.') }}đ</span></p>
                    </div>
                    <div>
                ...
                <div>
                    ...
                    <textarea name="ghi_chu" rows="2" maxlength="500"
                              ...
                              class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400 resize-none"></textarea>
                </div>
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden"
            ...
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                <div class="flex items-center gap-3">
                    ...
                    <h3 class="text-base font-bold text-gray-900">Thanh toán bằng MoMo</h3>
                </div>
            ...
            <div class="px-6 py-4 bg-gray-50 border-b border-gray-100">
                <div class="grid grid-cols-2 gap-3 text-sm">
                    <div>
                        <p class="text-xs text-gray-400 mb-0.5">Mã hóa đơn</p>
                        <p class="font-mono font-semibold text-gray-800">{{ $hoaDon->ma_thanh_toan }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-400 mb-0.5">Căn hộ</p>
                        <p class="font-semibold text-gray-800">{{ $hoaDon->canHo?->so_can_ho ?? '—' }}</p>
                    ...
                    <div>
                        <p class="text-xs text-gray-400 mb-0.5">Tổng tiền HĐ</p>
                        <p class="font-semibold text-gray-900 tabular-nums" x-text="fmtMoney(tongTienHoaDon)"></p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-400 mb-0.5">Kỳ</p>
                        <p class="font-semibold text-gray-800">{{ $hoaDon->thang }}/{{ $hoaDon->nam }}</p>
                    </div>
                </div>
                <div class="mt-3 pt-3 border-t border-gray-200 grid grid-cols-2 gap-3">
                    <div class="bg-emerald-50 rounded-lg px-3 py-2">
                        <p class="text-xs text-emerald-600 mb-0.5">Đã thanh toán</p>
                        <p class="font-bold text-emerald-700 tabular-nums" x-text="fmtMoney(daTTHoaDon)"></p>
                    </div>
                    <div class="bg-pink-50 rounded-lg px-3 py-2">
                        <p class="text-xs text-pink-600 mb-0.5">Còn phải thanh toán</p>
                        <p class="font-bold text-pink-700 tabular-nums" x-text="fmtMoney(getConNo())"></p>
                    </div>
                </div>
            </div>

            {{-- Form --}}
            <form x-ref="momoForm" method="POST" action="{{ route('manager.thanh-toan.momo', $hoaDon) }}" class="px-6 py-5 space-y-4">
                @csrf
                <div>
                    ...
                    <input type="number" name="so_tien"
                           ...
                           :class="momoError ? 'border-red-400' : 'border-gray-300'">
                    <p x-show="momoError" x-text="momoError" class="mt-1.5 text-xs text-red-500"></p>
                    <p x-show="!momoError" class="mt-1 text-xs text-gray-400">
                        Mặc định là toàn bộ số tiền còn nợ.
                        Tối đa: <span class="font-medium text-pink-600" x-text="fmtMoney(getConNo())"></span>
                    </p>
                </div>
                ...
                <div class="flex gap-2">
                    <button type="button"
                            ...
                            :disabled="momoLoading"
                            class="flex-1 py-1.5 text-xs font-medium rounded-lg border border-pink-200
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden"
            ...
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                <div class="flex items-center gap-3">
                    ...
                    <h3 class="text-base font-bold text-gray-900">Thanh toán bằng VNPay</h3>
                </div>
            ...
            <div class="px-6 py-4 bg-gray-50 border-b border-gray-100">
                ...
                <div class="mt-3 pt-3 border-t border-gray-200 grid grid-cols-2 gap-3">
                    ...
                    <div class="bg-blue-50 rounded-lg px-3 py-2">
                        <p class="text-xs text-blue-600 mb-0.5">Còn phải thanh toán</p>
                        <p class="font-bold text-blue-700 tabular-nums" x-text="fmtMoney(getConNo())"></p>
                    </div>
                </div>
            </div>

            {{-- Form --}}
            <form x-ref="vnpayForm" method="POST" action="{{ route('manager.thanh-toan.vnpay', $hoaDon) }}" class="px-6 py-5 space-y-4">
                @csrf
                <div>
                    <label class="block text-xs font-medium text-gray-700 mb-1.5">
                        Số tiền thanh toán (đ) <span class="text-red-500">*</span>
                    ...
                    <input type="number" name="so_tien"
                           ...
                           class="w-full px-3 py-2.5 border rounded-lg text-sm bg-white
                                  ...
                                  disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                           :class="vnpayError ? 'border-red-400' : 'border-gray-300'">
                    <p x-show="vnpayError" x-text="vnpayError" class="mt-1.5 text-xs text-red-500"></p>
                    <p x-show="!vnpayError" class="mt-1 text-xs text-gray-400">
                        Mặc định là toàn bộ số tiền còn nợ.
                        Tối đa: <span class="font-medium text-blue-600" x-text="fmtMoney(getConNo())"></span>
                    </p>
                </div>
                ...
                <div class="flex gap-2">
                    <button type="button"
                            ...
                            :disabled="vnpayLoading"
                            class="flex-1 py-1.5 text-xs font-medium rounded-lg border border-blue-200
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6"
            ...
            <div class="flex flex-col items-center text-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-500 mt-1">Dịch vụ sẽ bị loại khỏi hóa đơn và tổng tiền sẽ được cập nhật.</p>
                </div>
