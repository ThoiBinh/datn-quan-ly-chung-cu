...
<div class="max-w-3xl space-y-5"
    ...
    <div class="flex items-center gap-3 p-4 bg-amber-50 border border-amber-200 rounded-xl">
        ...
        <div>
            <p class="text-sm font-semibold text-amber-700">Hóa đơn đã thanh toán</p>
            <p class="text-xs text-amber-600 mt-0.5">Không thể chỉnh sửa hoặc xóa hóa đơn đã thanh toán. Chỉ xem thông tin.</p>
        </div>
    </div>
    @endif
    ...
    <div class="bg-indigo-50 border border-indigo-200 rounded-xl p-4">
        <div class="flex items-center justify-between">
            ...
            <div class="text-right">
                ...
                <p class="text-lg font-bold text-gray-900 tabular-nums" x-text="fmtTien(grandTotal)">{{ number_format($hoaDon->tong_tien ?? 0, 0, ',', '.') }}đ</p>
            </div>
        </div>
    </div>
    ...
    <form id="update-form" action="{{ route('manager.hoa-don.update', $hoaDon) }}" method="POST" class="space-y-5" @submit="chanNopNeuLoi($event)">
        @csrf @method('PUT')

        <!-- Status & deadline -->
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden {{ $isDaTT ? 'opacity-70' : '' }}">
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
            <div class="flex items-center gap-2.5 px-5 py-4 border-b border-gray-100">
                ...
                <div>
                    <h2 class="text-sm font-semibold text-gray-800">Thêm phí dịch vụ</h2>
                    <p class="text-xs text-gray-400 mt-0.5">Tick chọn dịch vụ chưa có trong hóa đơn để thêm nhanh</p>
                </div>
            </div>

            <div class="p-5 space-y-4">
                <template x-if="dsPhiMoi.length === 0">
                    <p class="text-sm text-gray-400">Không còn dịch vụ nào khác để thêm.</p>
                </template>

                <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    <template x-for="svc in dsPhiMoi" :key="svc.phi_dich_vu_id">
                        <label class="flex items-center gap-3 p-3 border rounded-xl cursor-pointer transition-colors border-gray-200 hover:border-emerald-300">
                            <input type="checkbox" name="phi_dich_vu_ids[]" :value="String(svc.phi_dich_vu_id)" x-model="checkedMoi"
                                   class="w-4 h-4 rounded border-gray-300 text-emerald-600 focus:ring-emerald-400">
                            ...
                            <div class="text-right flex-shrink-0">
                                ...
                                <p class="text-xs text-gray-400" x-text="svc.don_vi_tinh ? ('/' + svc.don_vi_tinh) : ''"></p>
                            </div>
                        </label>
                    </template>
                </div>
                ...
                <template x-if="selectedMoi.length > 0">
                    <div class="border border-emerald-200 rounded-lg overflow-hidden divide-y divide-emerald-100">
                        <template x-for="svc in selectedMoi" :key="'row-' + svc.phi_dich_vu_id">
                            <div class="px-4 py-3 bg-emerald-50/40">
                                <div class="flex items-center justify-between">
                                    <span class="text-sm font-medium text-gray-800" x-text="svc.ten_phi_dich_vu"></span>
                                    <span class="text-sm font-semibold text-gray-900 tabular-nums" x-text="fmtTien(thanhTienMoi(svc))"></span>
                                </div>
                                    ...
                                    <div class="grid grid-cols-3 gap-3 mt-2">
                                        <div>
                                            ...
                                            <input type="number" min="0" x-model.number="svc.chi_so_cu"
                                                   ...
                                                   class="w-full px-2 py-1.5 border border-gray-300 rounded-lg text-sm tabular-nums">
                                        </div>
                                        <div>
                                        ...
                                        <div>
                                            <p class="text-xs text-gray-400 mb-1">Tiêu thụ</p>
                                            <p class="text-sm font-medium text-gray-900 tabular-nums" x-text="soLuongMoi(svc)"></p>
                                        </div>
                                    </div>
                                </template>
                                <template x-if="svc.billing_type === 'area'">
                                    <p class="text-xs text-gray-400 mt-2">Diện tích: <span x-text="svc.so_luong"></span> m² (lấy từ thuộc tính căn hộ, không thể sửa)</p>
                                </template>
                                <template x-if="svc.billing_type !== 'meter' && svc.billing_type !== 'area'">
                                    <div class="mt-2 max-w-[140px]">
                                        ...
                                        <input type="number" min="0" step="any" x-model.number="svc.so_luong"
                                               ...
                                               :class="soLuongHopLe(svc) ? 'border-gray-300' : 'border-red-400'"
                                               class="w-full px-2 py-1.5 border rounded-lg text-sm tabular-nums">
                                        <template x-if="!soLuongHopLe(svc)">
                                            <p class="text-xs text-red-500 mt-1">Số lượng phải lớn hơn 0</p>
                                        </template>
                                    </div>
                                </template>
                            </div>
                        </template>
                        <div class="px-4 py-2.5 flex items-center justify-between bg-emerald-100/50">
                            <span class="text-xs font-medium text-emerald-700">Cộng thêm vào hóa đơn</span>
                            <span class="text-sm font-bold text-emerald-800 tabular-nums" x-text="fmtTien(tongThem)"></span>
                        </div>
                    </div>
                </template>
            </div>
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
            <div class="px-5 py-3 border-b border-gray-100">
                <h2 class="text-sm font-semibold text-gray-700">Chi tiết hóa đơn</h2>
                <p class="text-xs text-gray-400 mt-0.5">Chỉnh sửa số lượng, đơn giá, chỉ số hoặc xóa dịch vụ khỏi hóa đơn</p>
            </div>

            <template x-if="visibleRows.length === 0">
                <p class="text-sm text-gray-400 px-5 py-4">Hóa đơn chưa có dịch vụ nào.</p>
            </template>
                ...
                <template x-for="row in visibleRows" :key="row.chi_tiet_id">
                    <div class="px-5 py-4">
                        <div class="flex items-start justify-between gap-3">
                            <div class="min-w-0">
                                <span class="text-sm font-medium text-gray-800" x-text="row.ten_phi_dich_vu"></span>
                                <p class="text-xs text-gray-400 mt-0.5">Đơn giá: <span x-text="fmtTien(row.don_gia)"></span></p>
                            </div>
                            <div class="flex items-center gap-3 flex-shrink-0">
                                <span class="text-sm font-semibold text-gray-900 tabular-nums" x-text="fmtTien(thanhTienRow(row))"></span>
                        ...
                        <template x-if="row.is_meter">
                            <div class="grid grid-cols-3 gap-3 mt-2 max-w-sm">
                                ...
                                <div>
                                    <p class="text-xs text-gray-400 mb-1">Tiêu thụ</p>
                                    <p class="text-sm font-medium text-gray-900 tabular-nums" x-text="tieuThuRow(row)"></p>
                                </div>
                            </div>
                        </template>

                        <template x-if="!row.is_meter && row.billing_type === 'area'">
                            <p class="text-xs text-gray-400 mt-2">Diện tích: <span x-text="row.so_luong"></span> m² (lấy từ thuộc tính căn hộ, không thể sửa)</p>
                        </template>
                        ...
                        <template x-if="!row.is_meter && row.billing_type !== 'area'">
                            <div class="mt-2 max-w-[160px]">
                                ...
                                <input type="number" min="0" step="any" x-model.number="row.so_luong"
                                       ...
                                       :class="rowSoLuongHopLe(row) ? 'border-gray-300' : 'border-red-400'"
                                       class="w-full px-2 py-1.5 border rounded-lg text-sm tabular-nums">
        ...
        @else
            ...
            <!-- Other items (read-only; meter items not editable outside CHUA_THANH_TOAN) -->
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                <div class="px-5 py-3 border-b border-gray-100">
    ...
    <div class="flex items-center justify-between">
        ...
        <div class="relative group">
            ...
            <div class="absolute bottom-full left-0 mb-2 px-3 py-2 bg-gray-900 text-white text-xs rounded-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap z-10">
                ...
                <div class="absolute top-full left-4 -mt-0.5 w-2 h-2 bg-gray-900 rotate-45"></div>
            </div>
        </div>
        @else
        ...
        <div class="flex items-center gap-3">
            ...
            @else
            <button type="button" disabled
                    class="px-5 py-2.5 bg-indigo-300 text-white text-sm font-semibold rounded-lg cursor-not-allowed opacity-50"
