...
<div class="space-y-5" x-data="{ open: false, url: '', name: '' }" @open-delete.window="open = true; url = $event.detail.url; name = $event.detail.name">
        ...
        <div class="flex items-center gap-2 self-start lg:self-auto">
            <div class="relative" x-data="{ open: false }" @click.away="open = false">
                ...
                <div x-show="open" x-transition x-cloak
                    ...
                    <a href="{{ route('manager.tien-ich.index') }}" class="flex items-center gap-2 px-3 py-2 text-sm text-gray-700 dark:text-slate-300 hover:bg-gray-50 dark:hover:bg-slate-700 transition-colors">
                        <span>🏢</span>
                        Quản lý tiện ích
                    </a>
            ...
            <a href="{{ route('manager.dat-lich-tien-ich.dashboard') }}"
               class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl border border-gray-300 dark:border-slate-600 text-gray-700 dark:text-slate-200 text-sm font-semibold hover:bg-gray-50 dark:hover:bg-slate-700 transition-colors">
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm p-4">
        <form method="GET" action="{{ route('manager.dat-lich-tien-ich.index') }}" class="flex flex-wrap gap-2" x-data="{ loading: false }" @submit="loading = true">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-slate-700/30 transition-colors">
                        ...
                        <td class="px-4 py-3.5">
                            <div class="flex items-center gap-1 justify-end">
                                ...
                                <form action="{{ route('manager.dat-lich-tien-ich.approve', $item) }}" method="POST">
                                    @csrf @method('PATCH')
