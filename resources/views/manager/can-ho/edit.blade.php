...
<div class="max-w-2xl">
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm">
        <div class="p-5 border-b border-gray-200"><h2 class="font-semibold text-gray-800">Căn hộ {{ $canHo->so_can_ho }}</h2></div>
        <form method="POST" action="{{ route('manager.can-ho.update', $canHo) }}" class="p-5 space-y-4">
            @csrf @method('PUT')
            @if($errors->any())
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="toa_nha" required class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                        @foreach($toaNha as $tn)<option value="{{ $tn->id }}" {{ $canHo->toa_nha == $tn->id ? 'selected' : '' }}>{{ $tn->ten_toa_nha }}</option>@endforeach
                    </select>
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="number" name="tang" value="{{ old('tang', $canHo->tang) }}" required
                           class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                </div>
                <div>
                ...
                <div>
                    ...
                    <select name="trang_thai" required class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                        @foreach($trangThai as $tt)<option value="{{ $tt->id }}" {{ $canHo->trang_thai == $tt->id ? 'selected' : '' }}>{{ $tt->ten_trang_thai }}</option>@endforeach
                    </select>
                </div>
            </div>
