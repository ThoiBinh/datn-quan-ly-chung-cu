...
<div class="max-w-xl">
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm">
        <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
            <h2 class="text-base font-semibold text-gray-800">Chỉnh sửa vai trò</h2>
            <a href="{{ route('manager.vai-tro.show', $vaiTro) }}" class="text-sm text-gray-500 hover:text-gray-700 flex items-center gap-1">
        ...
        <form action="{{ route('manager.vai-tro.update', $vaiTro) }}" method="POST" class="p-6 space-y-5">
            @csrf @method('PUT')
            ...
            <div>
                ...
                <input type="text" name="vai_tro" value="{{ old('vai_tro', $vaiTro->vai_tro) }}"
                       class="w-full px-3 py-2 text-sm border rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500
                              {{ $errors->has('vai_tro') ? 'border-red-400 bg-red-50' : 'border-gray-200' }}"
                       autofocus/>
                @error('vai_tro')
            ...
            <div class="flex gap-3 pt-2">
                ...
                <a href="{{ route('manager.vai-tro.show', $vaiTro) }}"
