...
<div class="space-y-5">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-4 flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-xs text-gray-500">Tổng phí dịch vụ</p>
            </div>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-4 flex items-center gap-4">
            ...
            <div>
                <p class="text-2xl font-bold text-gray-800">{{ $tongDangSuDung }}</p>
                <p class="text-xs text-gray-500">Đang áp dụng căn hộ</p>
            </div>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-4 flex items-center gap-4">
            ...
            <div>
                <p class="text-2xl font-bold text-gray-800">{{ $tongCanHo }}</p>
                <p class="text-xs text-gray-500">Căn hộ đang dùng dịch vụ</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm">
        <div class="p-4 border-b border-gray-100 flex flex-col gap-3">
            <form method="GET" action="{{ route('manager.phi-dich-vu.index') }}" class="flex flex-wrap items-center gap-2">
                <div class="relative">
                ...
                <select name="loai_phi_dich_vu" class="px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500">
                ...
                <select name="don_vi_tinh" class="px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500">
                ...
                <select name="loai_tinh_phi" class="px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500">
                ...
                <a href="{{ route('manager.phi-dich-vu.index') }}" class="px-3 py-2 border border-gray-200 text-gray-500 text-sm rounded-lg hover:bg-gray-50">Xóa lọc</a>
                @endif
            </form>

            <div class="flex justify-end items-center justify-end gap-2">
               ...
               class="inline-flex items-center gap-2 px-3 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-medium rounded-lg transition-colors">
                ...
                <a href="{{ route('manager.phi-dich-vu.create') }}"
                   class="inline-flex items-center gap-2 px-3 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-medium rounded-lg transition-colors">
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="bg-gray-50 border-b border-gray-200">
                        ...
                        <th class="text-center px-4 py-3 font-semibold text-gray-600 text-xs uppercase tracking-wider">Căn hộ</th>
                        <th class="text-left px-4 py-3 font-semibold text-gray-600 text-xs uppercase tracking-wider">Người cập nhật</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        <td class="px-4 py-3 text-gray-400 text-xs font-mono">#{{ $phi->id }}</td>
                        <td class="px-4 py-3">
                        ...
                        <td class="px-4 py-3 text-gray-600 text-sm">{{ $phi->donViTinh?->don_vi ?? '—' }}</td>
                        <td class="px-4 py-3 text-gray-600 text-sm">{{ $phi->loaiTinhPhi?->ten_loai ?? '—' }}</td>
                        ...
                        <td class="px-4 py-3 text-center">
                            <span class="inline-flex items-center justify-center w-7 h-7 rounded-full text-xs font-semibold
                                {{ $phi->can_ho_count > 0 ? 'bg-indigo-100 text-indigo-700' : 'bg-gray-100 text-gray-500' }}">
                                {{ $phi->can_ho_count }}
                            </span>
                        </td>
                        <td class="px-4 py-3 text-gray-500 text-xs">{{ $phi->nguoiCapNhat?->ho_ten ?? '—' }}</td>
                        <td class="px-4 py-3 text-gray-400 text-xs whitespace-nowrap">
                            {{ $phi->createdAt ? \Carbon\Carbon::parse($phi->createdAt)->format('d/m/Y') : '—' }}
                        </td>
                        <td class="px-4 py-3">
                            <div class="flex items-center justify-center gap-1">
                                <a href="{{ route('manager.phi-dich-vu.show', $phi) }}"
                                   class="p-1.5 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Xem">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                </a>
                                <a href="{{ route('manager.phi-dich-vu.edit', $phi) }}"
                                   class="p-1.5 text-amber-600 hover:bg-amber-50 rounded-lg transition-colors" title="Sửa">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                </a>
                                @if($phi->can_ho_count === 0)
                                <button type="button"
                                        @click="$dispatch('open-delete', { url: '{{ route('manager.phi-dich-vu.destroy', $phi) }}', name: '{{ addslashes($phi->ten_phi_dich_vu) }}' })"
                                ...
                                @else
                                <span class="p-1.5 text-gray-300 cursor-not-allowed" title="Đang dùng trong {{ $phi->can_ho_count }} căn hộ — không thể xóa">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                                </span>
                                @endif
                            </div>
                    ...
                    <tr>
                        <td colspan="10" class="px-4 py-16 text-center">
                            <div class="flex flex-col items-center gap-2">
                                <svg class="w-10 h-10 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z"/></svg>
                                <p class="text-sm text-gray-400">Không tìm thấy phí dịch vụ nào</p>
                                <a href="{{ route('manager.phi-dich-vu.create') }}" class="text-sm text-indigo-600 hover:underline">Thêm mới</a>
                            </div>
                        </td>
                    </tr>
                    @endforelse
        ...
        @if($dsPhi->hasPages())
        <div class="px-4 py-3 border-t border-gray-100">{{ $dsPhi->links() }}</div>
        @endif
    </div>
