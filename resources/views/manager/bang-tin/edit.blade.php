...
<div class="max-w-2xl">
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm">
        ...
        <form method="POST" action="{{ route('manager.bang-tin.update', $bangTin) }}" class="p-5 space-y-4">
            @csrf @method('PUT')
            @if($errors->any())
            ...
            <div>
                ...
                <input type="url" name="hinh_url" value="{{ old('hinh_url', $bangTin->hinh_url) }}"
                       placeholder="https://..."
                       class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                @if($bangTin->hinh_url)
                <p class="mt-1 text-xs text-gray-400">URL hiện tại: <span class="font-mono break-all">{{ $bangTin->hinh_url }}</span></p>
                @endif
            </div>
            <div>
