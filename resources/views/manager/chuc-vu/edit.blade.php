...
@section('page-title', 'Sửa chức vụ')

@section('content')
<div class="max-w-xl">
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm">
        <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
            <h2 class="text-base font-semibold text-gray-800">Chỉnh sửa chức vụ</h2>
            <a href="{{ route('manager.chuc-vu.show', $chucVu) }}" class="text-sm text-gray-500 hover:text-gray-700 flex items-center gap-1">
        ...
        <form action="{{ route('manager.chuc-vu.update', $chucVu) }}" method="POST" class="p-6 space-y-5">
            @csrf @method('PUT')
            ...
            <div>
                ...
                <input type="text" name="chuc_vu" value="{{ old('chuc_vu', $chucVu->chuc_vu) }}"
                       class="w-full px-3 py-2 text-sm border rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500
                              {{ $errors->has('chuc_vu') ? 'border-red-400 bg-red-50' : 'border-gray-200' }}"
                       autofocus/>
                @error('chuc_vu')
            ...
            <div class="flex gap-3 pt-2">
                ...
                <a href="{{ route('manager.chuc-vu.show', $chucVu) }}"
