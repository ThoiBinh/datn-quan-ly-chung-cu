...
@section('title', 'Chỉnh sửa - ' . $nhanVien->ho_ten)
@section('page-title', 'Chỉnh sửa nhân viên')

@section('content')
<div class="max-w-4xl">
    <form method="POST" action="{{ route('manager.nhan-vien.update', $nhanVien) }}" class="space-y-5">
        @csrf
        @method('PUT')
        ...
        {{-- Header nhân viên --}}
        <div class="bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-200 dark:border-indigo-700 rounded-xl p-4 flex items-center gap-4">
            ...
            <div>
                <p class="font-semibold text-indigo-800 dark:text-indigo-200">{{ $nhanVien->ho_ten }}</p>
                <p class="text-xs text-indigo-600 dark:text-indigo-400">{{ $nhanVien->ma_nhan_vien }} · {{ $nhanVien->chucVu?->chuc_vu ?? '—' }}</p>
            </div>
        </div>
        ...
        <div class="flex gap-3">
            ...
            <a href="{{ route('manager.nhan-vien.show', $nhanVien) }}"
