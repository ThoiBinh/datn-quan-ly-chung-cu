...
@section('title', 'Chỉnh sửa phí dịch vụ căn hộ')
@section('page-title', 'Chỉnh sửa phí dịch vụ căn hộ')
...
<div class="max-w-4xl mx-auto space-y-5">
    ...
    <nav class="flex items-center gap-1.5 text-sm text-gray-500 dark:text-gray-400">
        ...
        <a href="{{ route('manager.can-ho-phi-dich-vu.show', $canHoPhiDichVu) }}" class="hover:text-indigo-600 transition-colors truncate max-w-[160px]">
            {{ $canHoPhiDichVu->canHo->so_can_ho ?? '?' }} — {{ $canHoPhiDichVu->phiDichVu->ten_phi_dich_vu ?? '?' }}
        </a>
        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
        <span class="text-gray-700 dark:text-gray-200">Chỉnh sửa</span>
    </nav>
    ...
    <form method="POST" action="{{ route('manager.can-ho-phi-dich-vu.update', $canHoPhiDichVu) }}"
          x-data="{
              canHoId: '{{ old('can_ho', $canHoPhiDichVu->can_ho) }}',
              phiDvId: '{{ old('phi_dich_vu', $canHoPhiDichVu->phi_dich_vu) }}',
              donGia: '{{ old('don_gia', $canHoPhiDichVu->don_gia) }}',
              canHoData: {{ $canHoJson }},
              phiDvData: {{ $phiDvJson }},
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
                <div class="px-5 py-4 border-b border-gray-100 dark:border-gray-700 flex items-center gap-2">
                    ...
                    <h2 class="text-sm font-semibold text-gray-800 dark:text-gray-100">Căn hộ</h2>
                </div>
                <div class="p-5 space-y-4">
                    <div>
                        ...
                        <select name="can_ho" x-model="canHoId"
                            ...
                            <option value="{{ $ch->id }}" {{ old('can_ho', $canHoPhiDichVu->can_ho) == $ch->id ? 'selected' : '' }}>
                    ...
                    <template x-if="selectedCanHo">
                        <div class="bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-800 rounded-lg p-3 space-y-1.5 text-xs">
                            <div class="flex justify-between">
                                ...
                                <span class="font-medium text-gray-800 dark:text-gray-200" x-text="selectedCanHo.toa_nha_ten ?? '—'"></span>
                            </div>
                            <div class="flex justify-between">
                            ...
                            <div class="flex justify-between">
                                <span class="text-gray-500 dark:text-gray-400">Loại căn hộ</span>
                                <span class="font-medium text-gray-800 dark:text-gray-200" x-text="selectedCanHo.loai_can_ho ?? '—'"></span>
                            </div>
                        </div>
                    </template>
                </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
                <div class="px-5 py-4 border-b border-gray-100 dark:border-gray-700 flex items-center gap-2">
                    ...
                    <h2 class="text-sm font-semibold text-gray-800 dark:text-gray-100">Phí dịch vụ</h2>
                </div>
                <div class="p-5 space-y-4">
                    <div>
                        ...
                        <select name="phi_dich_vu" x-model="phiDvId" @change="onPhiDvChange()"
                            ...
                            <option value="{{ $pdv->id }}" {{ old('phi_dich_vu', $canHoPhiDichVu->phi_dich_vu) == $pdv->id ? 'selected' : '' }}>
            ...
            <div class="p-5">
                <div class="max-w-xs">
                    ...
                    <input type="number" name="don_gia" x-model="donGia" min="0" step="0.01"
                           value="{{ old('don_gia', $canHoPhiDichVu->don_gia) }}"
        ...
        <div class="flex items-center justify-end gap-3">
            <a href="{{ route('manager.can-ho-phi-dich-vu.show', $canHoPhiDichVu) }}"
