...
<div class="max-w-2xl">
    ...
    <nav class="flex items-center gap-2 text-sm text-gray-500 mb-5">
        ...
        <a href="{{ route('manager.cu-dan-can-ho.show', $cuDanCanHo) }}" class="hover:text-indigo-600 transition-colors">#{{ $cuDanCanHo->id }}</a>
        <svg class="w-3.5 h-3.5 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
        </svg>
    ...
    <div class="bg-white rounded-2xl border border-gray-200 shadow-sm">
        ...
        <form method="POST" action="{{ route('manager.cu-dan-can-ho.update', $cuDanCanHo) }}" class="px-6 py-5 space-y-5">
            @csrf @method('PUT')
            ...
            <div>
                ...
                <select name="vai_tro" id="vai-tro-select"
                    ...
                    <option value="{{ $vt->id }}" data-chu-ho="{{ $vt->vai_tro === 'Chủ sở hữu' ? '1' : '0' }}" {{ old('vai_tro', $cuDanCanHo->vai_tro) == $vt->id ? 'selected' : '' }}>
                        {{ $vt->vai_tro }}
                    </option>
                    @endforeach
                </select>
            ...
            <div class="flex items-center gap-3 pt-2 border-t border-gray-100">
                ...
                <a href="{{ route('manager.cu-dan-can-ho.show', $cuDanCanHo) }}"
