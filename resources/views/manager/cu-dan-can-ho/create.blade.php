...
@section('title', 'Thêm cư dân căn hộ')

@section('content')
<div class="max-w-2xl">
    {{-- Breadcrumb --}}
    <nav class="flex items-center gap-2 text-sm text-gray-500 mb-5">
        <a href="{{ route('manager.cu-dan-can-ho.index') }}" class="hover:text-indigo-600 transition-colors">Cư dân - Căn hộ</a>
    ...
    <div class="bg-white rounded-2xl border border-gray-200 shadow-sm">
        <div class="px-6 py-5 border-b border-gray-100">
            <h1 class="text-lg font-bold text-gray-800">Thêm thông tin cư trú</h1>
            <p class="text-sm text-gray-500 mt-0.5">Gán cư dân vào căn hộ với vai trò và thời gian cư trú.</p>
        </div>

        <form method="POST" action="{{ route('manager.cu-dan-can-ho.store') }}" class="px-6 py-5 space-y-5">
            @csrf
            ...
            <div>
                ...
                <select name="can_ho" id="can-ho-select" required
            ...
            <div>
                ...
                <select name="vai_tro" id="vai-tro-select"
                    ...
                    <option value="{{ $vt->id }}" data-chu-ho="{{ $vt->vai_tro === 'Chủ sở hữu' ? '1' : '0' }}" {{ old('vai_tro') == $vt->id ? 'selected' : '' }}>
                        {{ $vt->vai_tro }}
                    </option>
                    @endforeach
                </select>
                @error('vai_tro')<p class="mt-1 text-xs text-red-500">{{ $message }}</p>@enderror
                <p id="chu-ho-error" class="mt-1 text-xs text-red-500 hidden"></p>
            </div>

            {{-- Ngày chuyển đến / Ngày chuyển đi --}}
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">Ngày chuyển đến</label>
                    <input type="date" name="ngay_chuyen_den" value="{{ old('ngay_chuyen_den') }}"
                ...
                <div>
                    ...
                    <input type="date" name="ngay_chuyen_di" value="{{ old('ngay_chuyen_di') }}"
            ...
            <div>
                ...
                <select name="trang_thai" required
                    ...
                    <option value="{{ $val }}" {{ old('trang_thai', 1) == $val ? 'selected' : '' }}>
                        {{ $label['text'] }}
                    </option>
                    @endforeach
                </select>
            ...
            <div class="flex items-center gap-3 pt-2 border-t border-gray-100">
                <button type="submit" id="btn-luu"
                        class="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold rounded-xl transition-colors disabled:opacity-60 disabled:cursor-not-allowed">
                    Lưu thông tin
                </button>
                <a href="{{ route('manager.cu-dan-can-ho.index') }}"
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        if (!window.ChuHoCheck) return;
        window.ChuHoCheck.init({
            canHoSelect: document.getElementById('can-ho-select'),
            vaiTroSelect: document.getElementById('vai-tro-select'),
            submitBtn: document.getElementById('btn-luu'),
            errorBox: document.getElementById('chu-ho-error'),
            checkUrl: '{{ route('manager.cu-dan-can-ho.kiem-tra-chu-ho') }}',
            ignoreId: null,
        });
    });
</script>
