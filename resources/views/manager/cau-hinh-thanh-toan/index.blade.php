        ...
        <form method="GET" action="{{ route('manager.cau-hinh-thanh-toan.index') }}" class="flex flex-wrap gap-2">
            <div class="relative">
                ...
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Tìm ngân hàng, chủ TK, partner code, TMN code..."
                       class="pl-9 pr-4 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 w-80"/>
            </div>
            ...
            @if(request()->hasAny(['search','trang_thai']))<a href="{{ route('manager.cau-hinh-thanh-toan.index') }}" class="px-3 py-2 border border-gray-200 text-gray-500 text-sm rounded-lg hover:bg-gray-50">×</a>@endif
        </form>
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center justify-center gap-1">
                                    ...
                                    <button type="button"
                                        @click="$dispatch('open-toggle', { url: '{{ route('manager.cau-hinh-thanh-toan.toggle-status', $ch) }}', isActive: {{ $ch->trang_thai ? 'true' : 'false' }} })"
                                        class="p-1.5 rounded-lg {{ $ch->trang_thai ? 'text-red-400 hover:bg-red-50' : 'text-emerald-500 hover:bg-emerald-50' }}"
