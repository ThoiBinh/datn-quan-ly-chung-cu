...
<div class="max-w-2xl">
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm">
        <div class="p-5 border-b border-gray-200"><h2 class="font-semibold text-gray-800">Chỉnh sửa thông báo</h2></div>
        <form method="POST" action="{{ route('manager.thong-bao.update', $thongBao) }}" class="p-5 space-y-4">
            ...
            <div>
                ...
                <textarea name="noi_dung" rows="10" required
                          class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-y">{{ old('noi_dung', $thongBao->noi_dung) }}</textarea>
            </div>
            <div class="flex gap-3 pt-2">
                ...
                <a href="{{ route('manager.thong-bao.show', $thongBao) }}" class="flex-1 py-2.5 border border-gray-300 text-gray-700 text-sm font-medium rounded-lg hover:bg-gray-50 text-center">Hủy</a>
            </div>
        </form>
    </div>
</div>
