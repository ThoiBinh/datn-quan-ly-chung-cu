        ...
        <form method="POST" action="{{ route('manager.thong-bao.store') }}" class="p-5 space-y-4">
            ...
            <div>
                ...
                <textarea name="noi_dung" rows="8" required
                          class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none">{{ old('noi_dung') }}</textarea>
            </div>
            <div class="flex gap-3 pt-2">
                <button type="submit" class="px-6 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium rounded-lg">Đăng thông báo</button>
