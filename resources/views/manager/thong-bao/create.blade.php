...
<div class="max-w-2xl">
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm">
        <div class="p-5 border-b border-gray-200"><h2 class="font-semibold text-gray-800">Nội dung thông báo</h2></div>
        <form method="POST" action="{{ route('manager.thong-bao.store') }}" class="p-5 space-y-4">
            @csrf
            @if($errors->any())
            ...
            <div class="flex gap-3 pt-2">
                <button type="submit" class="flex-1 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold rounded-lg">Đăng thông báo</button>
                <a href="{{ route('manager.thong-bao.index') }}" class="flex-1 py-2.5 border border-gray-300 text-gray-700 text-sm font-medium rounded-lg hover:bg-gray-50 text-center">Hủy</a>
            </div>
        </form>
    </div>
</div>
