...
<div class="max-w-2xl mx-auto space-y-5" x-data="{ preview: {{ $tienIch->hinh_url ? \Illuminate\Support\Js::from(asset('storage/' . $tienIch->hinh_url)) : 'null' }}, removeImg: false }">
    ...
    <nav class="flex items-center gap-1.5 text-sm text-gray-500 dark:text-slate-400">
        ...
        <a href="{{ route('manager.tien-ich.show', $tienIch) }}" class="hover:text-indigo-600 transition-colors truncate max-w-xs">{{ $tienIch->ten_tien_ich }}</a>
        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
    ...
    <form id="update-form" action="{{ route('manager.tien-ich.update', $tienIch) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
        @csrf @method('PUT')
    ...
    <div class="flex items-center justify-between">
        <a href="{{ route('manager.tien-ich.show', $tienIch) }}"
