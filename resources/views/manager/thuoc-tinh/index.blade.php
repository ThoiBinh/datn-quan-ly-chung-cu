...
<div class="space-y-5">
    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm">
        <div class="p-4 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <form method="GET" action="{{ route('manager.thuoc-tinh.index') }}" class="flex flex-wrap items-center gap-2">
                <div class="relative">
                ...
                <a href="{{ route('manager.thuoc-tinh.index') }}" class="px-3 py-2 border border-gray-200 text-gray-500 text-sm rounded-lg hover:bg-gray-50">Xóa lọc</a>
                @endif
            </form>

            <a href="{{ route('manager.thuoc-tinh.create') }}"
                    ...
                    <tr class="hover:bg-gray-50 transition-colors {{ $item->deletedAt ? 'opacity-60' : '' }}">
                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center justify-center gap-1">
                                @if($item->deletedAt)
                                <form action="{{ route('manager.thuoc-tinh.restore', $item->id) }}" method="POST">
                                    @csrf @method('PATCH')
                                ...
                                @else
                                <a href="{{ route('manager.thuoc-tinh.show', $item) }}" class="p-1.5 text-blue-600 hover:bg-blue-50 rounded-lg" title="Xem">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                                    </svg>
                                </a>
                                <a href="{{ route('manager.thuoc-tinh.edit', $item) }}" class="p-1.5 text-amber-600 hover:bg-amber-50 rounded-lg" title="Sửa">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                    </svg>
                                </a>
                                <button type="button"
                                        @click="$dispatch('open-delete', { url: '{{ route('manager.thuoc-tinh.destroy', $item) }}', name: '{{ addslashes($item->ten_thuoc_tinh) }}' })"
