...
class ThanhToanController extends Controller
{
    ...
    public function initiateMomo(Request $request, HoaDon $hoaDon)
    {
        ...
        session([
            'momo_show_url'  => route('manager.hoa-don.show', $hoaDon),
            'momo_index_url' => route('manager.hoa-don.index'),
        ]);
        ...
        try {
            ...
            Log::warning('[MoMo Manager] No payUrl in response', ['result' => $result, 'hoaDon' => $hoaDon->id]);
            return back()->with('error', 'MoMo: ' . ($result['message'] ?? 'Không thể tạo giao dịch MoMo.'));

        } catch (\Exception $e) {
            Log::error('[MoMo Manager] Exception', ['message' => $e->getMessage(), 'hoaDon' => $hoaDon->id]);
    ...
    public function initiateVnpay(Request $request, HoaDon $hoaDon)
    {
        ...
        session([
            'vnpay_show_url'  => route('manager.hoa-don.show', $hoaDon),
            'vnpay_index_url' => route('manager.hoa-don.index'),
        ]);

        try {
            Log::info('[VNPay Manager] Creating payment URL', ['txnRef' => $txnRef, 'amount' => $amount, 'hoaDon' => $hoaDon->id]);
            $payUrl = $this->vnpayService->taoUrlThanhToan($txnRef, $amount, $orderInfo);
            return redirect($payUrl);
        } catch (\Exception $e) {
            Log::error('[VNPay Manager] Exception', ['message' => $e->getMessage(), 'hoaDon' => $hoaDon->id]);
