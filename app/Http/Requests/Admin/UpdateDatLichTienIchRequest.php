...
class UpdateDatLichTienIchRequest extends FormRequest
{
    ...
    {
        $id = $this->route('datLichTienIch')?->id;

        return [
            'ma_dat_lich'     => [
                'required', 'string', 'max:50',
                Rule::unique('dat_lich_tien_ich', 'ma_dat_lich')->whereNull('deletedAt')->ignore($id),
            ],
